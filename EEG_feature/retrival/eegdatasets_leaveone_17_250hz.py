...
img_directory_training = "/mnt/repo0/ldy/4090_THINGS/images_set/training_images"#config["img_directory_training"]
img_directory_test = "/mnt/repo0/ldy/4090_THINGS/images_set/test_images" #config["img_directory_test"]
    ...
    def load_data(self):
        ...
        for subject in self.subjects:
            if self.train:
                ...
                file_name = 'preprocessed_eeg_training.npy' #'preprocessed_eeg_training.npy'

                file_path = os.path.join(self.data_path, subject, file_name)
                # file_path = '/mnt/repo0/kyw/encoding_eeg/preprocessed_data/sub-08/preprocessed_eeg_training.npy'
                # data = np.load(file_path, allow_pickle=True).item()
                data = np.load(file_path, allow_pickle=True)
                print("Data keys:", data.keys())
                
                # preprocessed_eeg_data = torch.from_numpy(data['preprocessed_eeg_data']).float().detach()     
                # print(preprocessed_eeg_data.shape)  # 打印形状确认是否正确加载           
                ...
                preprocessed_eeg_data = torch.from_numpy(data['preprocessed_eeg_data'][:, :, selected_indices, :]).float().detach()   
                print(preprocessed_eeg_data.shape)  # 打印形状确认是否正确加载      
                ...
                if subject == self.exclude_subject or self.exclude_subject==None:  # 跳过被排除的被试                    :  # 跳过被排除的被试                                        
                    ...
                    file_path = os.path.join(self.data_path, subject, file_name)
                    # file_path = '/mnt/repo0/kyw/encoding_eeg/preprocessed_data/sub-08/preprocessed_eeg_test.npy'
...
if __name__ == "__main__":
    ...
    data_path = "/mnt/repo0/kyw/encoding_eeg/Preprocessed_data_250Hz"  # Replace with the path to your data
    data_path = data_path

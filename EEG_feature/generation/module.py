...
def main_experiment_loop(seed, save_path):
    ...
    model_path = '/mnt/repo0/kyw/close-loop/sub_encoder_alexnet/sub-10/model_state_dict.pt'
    ...
    target_eeg_path = '/mnt/repo0/kyw/close-loop/sub_encoder_alexnet_test/sub-10/00183_tick_183.npy'
...
if __name__ == "__main__":
    ...
    for i in range(1, num_run + 1):
        base_save_path = f'/mnt/repo0/kyw/close-loop/10_1/exp7/loop_random_{i}'
        save_path = f"{base_save_path}{i + 1}"
    for i in range(300, num_run):
        save_path = f"{base_save_path}{i + 1}"
        os.makedirs(save_path, exist_ok=True)
        print(f'run{i + 1}/{num_run}')
        main_experiment_loop(i, save_path)